...
var _proto_ = {
	...
	render: function(token, data, callback) {
		...
		var subJsonDataFiles=[],targetSubDir=sysPath.join(config.templateDir,"module",sysPath.relative("page",token));
		console.log("targetSubDir=%s",targetSubDir);
		utils.walkSync(targetSubDir,subJsonDataFiles,/\.tpl$/);

		subJsonDataFiles.forEach(function(item,index){

			var dataPath=sysPath.join(config.dataDir,sysPath.relative(config.templateDir,item)).replace(/\.tpl$/,'.json');
		...
		//2013-11-19[11:33:44]template/module下的公共module
		var commonModuleList=fs.readdirSync(sysPath.join(config.templateDir,"module"));
		//console.log(commonModuleList);
		commonModuleList.forEach(function(item) {
			var stats = fs.statSync(sysPath.join(sysPath.join(config.templateDir,"module"), item));
			if (stats.isFile()) {
				
				//如果filter存在并为正则表达式，则过滤文件名
				if (/\.tpl$/.test(item)&&!/parent\.tpl/.test(item)){
					//read json
					var path=sysPath.join(sysPath.join(config.templateDir,"module"), item);
