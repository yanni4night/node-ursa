        ...
        return content.replace(reg, function(k) {
            ...
            if (undefined === parsedUrl.query[timeStampName]) {
                ...
                if (hasProtocol) {
                    ...
                    var filePath = systemPath.join(self.staticResDir, parsedUrl.pathname || parsedUrl.pathname);
        ...
        return content.replace(/url\(['"]?(.*?)['"]?\)/img, function(k) {
            ...
            if (undefined === parsedUrl.query[timeStampName]) {
                ...
                if (hasProtocol) {
                    ...
                    //图片途径相对于CSS文件
                    var imgFilePath = systemPath.join(baseDir, parsedUrl.pathname || parsedUrl.path || "");
                    //计算时间戳，文件不存在则置空
