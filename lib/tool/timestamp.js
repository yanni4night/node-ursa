...
var _proto_ = {
     ...
     * @return {Sting} 合法化的时间戳字段名.
     */
    _getTimeStampName: function() {
        var timeStampName = config.get("timestamp_name") || "t";
        if (/[&\?\s]/.test(timeStampName)) {
            logger.logError("Invalid timestamp_name:%s".error, timeStampName);
            return 't';
        }
    ...
    _addToHtmlCommon: function(content, reg) {
        ...
        return content.replace(reg, function(k) {
            ...
            if (undefined === parsedUrl.query[timeStampName]) {
               ...
               logger.logWarn("[DUMPLICATE]%s may already have a timeStamp".warn, parsedUrl.pathname);
            }
            return k;
...
Stamp.getTimeStamp = function(file) {
    ...
    try {
        ...
        logger.logError("[FATAL]getTimeStamp failed:%s".error, e)
        return "";
    }
};
