	 ...
	 proxys : [{
		test: function(pattern, redirectUrl, req) {
			...
			try {
				...
				console.error("%s is not a valid regexp pattern.".error, pattern.slice(6));
				return false;
			}
		...
		constructUrl: function(pattern, redirectUrl, req) {
			...
			var parsedReqUrl = sysUrl.parse(req.path, true);
		...
		test: function(pattern, redirectUrl, req) {
			return /^exact:/i.test(pattern) && (pattern.slice(6) == req.path);
		},
		constructUrl: function(pattern, redirectUrl, req) {
			return redirectUrl;
		}
		...
		test: function(pattern, redirectUrl, req) {
			return req.path.indexOf(pattern) > 0;
		},
		constructUrl: function(pattern, redirectUrl, req) {
			return redirectUrl;
		}
