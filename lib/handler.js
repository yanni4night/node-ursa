 ...
 * 2013-11-24[00:58:27]:module supported
 *
...
module.exports = {
    ...
    index: function(req, res) {
        ...
        if (!util.isArray(htmlFiles)) {
            logger.logWarn("'require_html_modules[%s]' IS NOT an array,search all.".warn, typeof htmlFiles);
            htmlFiles = [];
        ...
        //遍历构建模板的完整路径
        htmlFiles = htmlFiles.filter(function(file, index) {
            //file可能带.tpl后缀也可能不带
            ...
            if (!fs.existsSync(realPath)) {
                ...
                return false;
            } else if (new RegExp("^\/?"+(config.get('common_dir')||'_common').replace(/^[\.\/]*/,''),'')/*/^\/?_common/*/.test(file)) {
                logger.logWarn("%s should be hidden".warn, realPath);
                //htmlFiles.splice(index, 1);
                return false;
            }
            return true;
    ...
    tpl: function(req, res) {
        ...
        if (/*/^\/_common/*/new RegExp("^\/?"+(config.get('common_dir')||'_common').replace(/^[\.\/]*/,''),'').test(pathname))
        ...
        return render.render(pathname, {
            ...
            //try to reg module
            //2013-11-23[16:59:44]
            //嗅探是否是模块（不包含HTML和BODY标签）
            if (!/<(body|html)/ig.test(content)) {

                logger.logInfo("Detect a module,wrap a parent".info);

                var css_deps = content.match(/@require\s+(['"])?[\w\.\/\\-]+\.css\b\1?/g) || [];
                css_deps.forEach(function(item, index) {
                    var css = item.match(/[\w\.\/\\-]+\.css\b/g)[0];
                    css_deps[index] = css;
                });

                css_deps = utils.arrayUniq(css_deps);

                return render._renderFile(sysPath.join(config.templateDir, '/_module/parent.tpl'), {
                    name: pathname,
                    content: content,
                    required_css: css_deps
                }, function(err, content) {
                    if (!err) {
                        //移除所有包含@require的注释
                        content = content.replace(/<!--[\s\S]*?@require[\s\S]*?-->/img, '');
                        return res.send(content);
                    } else
                        return res.send(500, String(err));
                });
