...
module.exports={
    ...
    index:function(req,res){
     ...
     //  var indexFile=sysPath.join(__dirname,"../","template/index.tpl");
        ...
        var content=render.render('index',{tpls:htmlFiles});
...
/*        var tplFile=sysPath.join(config.templateDir,url.replace(/ut$/,config.templateSuffix));
        var dataFile=sysPath.join(config.dataDir,url.replace(/ut$/,'json'));
        ...
        //put a _token here
        var _token = url.replace(/(^\/*|\.\w*$)/g, "").replace(/\//g, '_');
    ...
    m: function(req, res) {
        ...
        //要输出的json
        var dataFile =sysPath.join(config.dataDir,pathname.replace(/m$/, 'json'));

        return fs.readFile(dataFile, function(err, json) {
            json = json || "";
            var content=render.render('mgr',{data:json, tpl: sysPath.join(".",pathname.replace(/m$/, 'json'))});
