 ...
 *  2013/11/23:add 'arrayUniq'&'mergeObject' functions
 *
...
var _Utils = {
  ...
  /**
   * Merge Object from src to dest.
   * @param  {Object} src  [description]
   * @param  {Object} dest [description]
   * @param  {Boolean} dest [description]
   * @return {Object}      dest
   */
  mergeObject: function(src, dest,override) {

    if ('object' != typeof src ||'object' != typeof dest||src===null||dest===null||typeof src['split']=='function') {
        return override?src:(dest?dest:src);
    }
      for (var e in src) {
        if (!src.hasOwnProperty(e)) continue;
        var tmp=this.mergeObject(src[e], dest[e],override);
        if(tmp){dest[e] = tmp;}
      }
